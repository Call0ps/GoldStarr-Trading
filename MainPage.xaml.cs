...
namespace GoldStarr_Trading
{
    ...
    public sealed partial class MainPage : Page, INotifyPropertyChanged, IMessageToUser
    {
        ...
        private void AddSupplierItemContent_Click(object sender, RoutedEventArgs e)
        {
            var parent = (sender as Button).Parent;
            ...
            Supplier showSupplier = _app.Suppliers.First(x => x.SupplierName == supplier.ToString());

            if (supplier == null || stockOrder == null)
            {
                MessageToUser("You must choose a supplier and an item");
            }
            else if (orderQuantity == "" || orderQuantity == "" || amount == 0)
            {
                MessageToUser("You must enter an integer");
            ...
            else
            {
                ...
                //if (orderQuantity != "" && orderQuantity != " " && orderQuantity != 0)
                //{

                // if no orders are present, simply add an order to the collection.
                if (_app.NewItems.Count == 0)
                {
                    //store.RemoveFromStock(stockOrder, amount);

                    StockClass order = new StockClass(stockOrder.ItemName, stockOrder.Supplier, amount);

                    _app.GetDefaultDeliverysList().Add(order);

                    //store.CreateOrder(customerOrderer, order);

                    //MessageToUser($"You have successfully created a new Customer order for: \n{customerOrderer.CustomerName} with {amount} {stockOrder.ItemName} in it");
                    MessageToUser($"You have successfully added a new Supplier item \n\nSupplier: {supplier.SupplierName} \nItem: {order.ItemName} \nAmount: {order.Qty}");

                    //SupplierTabSuppliersComboBox.SelectedIndex = -1;
                    //SupplierTabItemComboBox.SelectedIndex = -1;
                    //SupplierOrderQuantity.Text = "";
        ...
        private void AddNewItemTabSuppliersComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            string supplierName = e.AddedItems[0].ToString();
